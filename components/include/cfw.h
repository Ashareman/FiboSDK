...
typedef struct
{
    ...
    uint16_t bw;
#ifdef CONFIG_SUPPORT_EXTRA_FUNCTION
    int16_t pwr; //pcl
    uint16_t TxRxmode;
    uint16_t TxRx;
    uint16_t onoff;
...
/*! \brief The CFW_PBKI2Buf function convert NV_ME_PHB_INFOV2 to buf
\param [in]   pbki Pointer to NV_ME_PHB_INFOV2 type to store pbk info
\param [out]  buf  Pointer to uint8_t type
*/
void CFW_PBKI2Buf(NV_ME_PHB_INFOV2 *pbki, uint8_t *buf);
/*! \brief The CFW_Buf2PBKI function convert buf to NV_ME_PHB_INFOV2
\param [out] pbki Pointer to NV_ME_PHB_INFOV2 type to store pbk info
\param [in]  buf  Pointer to uint8_t type
*/
void CFW_Buf2PBKI(uint8_t *buf, NV_ME_PHB_INFOV2 *pbki);
/*! \brief The CFW_NV_PBK_Init function init the phone book
\return
If the function succeeds, the return value is ERR_SUCCESS, if the function fails, the following error code may be returned.
\rst
+----------------------------+------------------------------------------+
...
\endrst
*/
int32_t CFW_NV_PBK_Init(void);
/*! \brief The CFW_NV_READ_ME_PBK function get the ME PBK infomation
\param [in]  iIndex  the index of the phone book
\param [out] pBuff Pointer to uint8_t type to get ME buf
\return
If the function succeeds, the return value is ERR_SUCCESS, if the function fails, the following error code may be returned.
\rst
+----------------------------+------------------------------------------+
...
\endrst
*/
int32_t CFW_NV_READ_ME_PBK(uint32_t iIndex, uint8_t *pBuff);
/*! \brief The CFW_NV_WRITE_ME_PBK function write PBK infomation to ME
\param [in]  iIndex  the index of the phone book
\param [in]  pBuff   Pointer to uint8_t type,save the pbk info that write to ME
\param [in]  type    add/del to ME
\return
If the function succeeds, the return value is ERR_SUCCESS, if the function fails, the following error code may be returned.
\rst
+----------------------------+------------------------------------------+
...
\endrst
*/
int32_t CFW_NV_WRITE_ME_PBK(uint32_t iIndex, uint8_t *pBuff, uint8_t type);
/*! \brief The CFW_NV_GET_STORGE_ME_PBK function to get the ME current using state
\param [in]  iMaxEntries  MAX_ME_PHONE_ENTRIES
\return
using number of the ME storage
*/
int32_t CFW_NV_GET_STORGE_ME_PBK(uint32_t iMaxEntries);
...
|        |If nOption==2 nParam1 is NULL.     |0x101，ussd_v2�?x100).       |                |
+--------+-----------------------------------+------------------------------+----------------+
...
typedef enum
{
	...
	uint32_t CFW_GprsSendCtxCfg_V2(CFW_GPRS_DFTPDN_INFO *nDftPdn,CFW_SIM_ID sSimID);
#endif
...
uint32_t CFW_SetPowerPriority(uint8_t iPriority, CFW_SIM_ID nSimID);
uint32_t CFW_GetPowerPriority(uint8_t *iPriority, CFW_SIM_ID nSimID);
#endif
...
#ifdef CONFIG_SUPPORT_EXTRA_FUNCTION
uint16_t sim_GetSimCardModeCB(void);
