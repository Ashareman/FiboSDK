...
// <!husongyong@fibocom  added for License
#ifdef CONFIG_FIBOCOM_BASE
#define MAX_LICENSE_LEN (512)
typedef uint8_t nvmLicense_t[MAX_LICENSE_LEN];
#endif
//end-->
#ifdef CONFIG_FIBO_MIDEA_ENABLE
#define MIDEA_LICENSE_LEN (2048)
typedef uint8_t mideaLicense_t[MIDEA_LICENSE_LEN];
#endif
/*read param from version.bin*/
typedef enum enum_nwtype
{
	gsm_and_lte = 0,
    lte_only = 1,
    tdd_only = 2,
...
#if defined(CONFIG_MG612_CN_20_00) || defined(CONFIG_MG612_CN_20_20_TIANYU)
...
#ifdef CONFIG_FIBOCOM_BASE
int nvmReadLicense(int index, nvmLicense_t *license);
int nvmWriteLicense(int index, const nvmLicense_t *license);
#endif

#ifdef CONFIG_FIBO_MIDEA_ENABLE
int mideaReadLicense(mideaLicense_t *license);
int mideaWriteLicense(const mideaLicense_t *license);
#endif
...
int nvmReadTestFlag(uint16_t *com_flag, uint16_t *coup_flag);
int nvmReadHWversion(uint32_t *verno);
#endif
