...
typedef void* osiEventGroupHandle_t;

typedef uint32_t osiEventBits_t;

typedef long osiBaseType_t;

typedef uint32_t osiTickType_t;

typedef void* osiTaskHandle_t;

typedef enum
{
    osiNoAction = 0,              /* Notify the task without updating its notify value. */
    osiSetBits,                   /* Set bits in the task's notification value. */
    osiIncrement,                 /* Increment the task's notification value. */
    osiSetValueWithOverwrite,     /* Set the task's notification value to a specific value even if the previous value has not yet been read by the task. */
    osiSetValueWithoutOverwrite   /* Set the task's notification value if the previous value has been read by the task. */
} osiNotifyAction_t;
...
osiEventGroupHandle_t osiEventGroupCreate();

osiEventBits_t osiEventGroupSetBits(osiEventGroupHandle_t eventGroup, const osiEventBits_t bitsToSet);

osiEventBits_t osiEventGroupWaitBits(osiEventGroupHandle_t eventGroup, const osiEventBits_t bitsToWaitFor, const osiBaseType_t clearOnExit, const osiBaseType_t waitForAllBits, osiTickType_t ticksToWait);

osiBaseType_t osiEventGroupSetBitsFromISR(osiEventGroupHandle_t eventGroup, const osiEventBits_t bitsToSet, osiBaseType_t *higherPriorityTaskWoken);

osiEventBits_t osiEventGroupClearBits(osiEventGroupHandle_t eventGroup, const osiEventBits_t bitsToClear);

uint32_t osiTaskNotifyTake(osiBaseType_t clearCountOnExit, osiTickType_t ticksToWait);

osiBaseType_t osiTaskNotifyWait(uint32_t bitsToClearOnEntry, uint32_t bitsToClearOnExit, uint32_t *pNotificationValue, osiTickType_t ticksToWait);

osiBaseType_t osiTaskGenericNotify(osiTaskHandle_t taskToNotify, uint32_t value, osiNotifyAction_t eAction, uint32_t *pPreviousNotificationValue);
