...
typedef struct
{
	...
	bool is16_bit;
	bool is_use; //inside address or not
#else
    uint8_t addr_data;
#endif
} drvI2cSlave_t;
...
#ifdef CONFIG_LG610_CN_10_92_2048_MIPI
/**
 * @brief i2c slave definition
 */
typedef struct
...
bool drvI2cRead_32bit(drvI2cMaster_t *d, const drvI2cSlave_N *slave, uint8_t *buf, uint32_t length);
bool drvI2cWrite_32bit(drvI2cMaster_t *d, const drvI2cSlave_N *slave, const uint8_t *data, uint32_t length);
