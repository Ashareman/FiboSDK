...
static void savetoSD(char *filename,uint8_t *in_buff,uint32_t len)
{
    ...
    if (fd<0)
    {
        OSI_PRINTFI("[%s %d] audio open %s error numer %d",__FUNCTION__, __LINE__, filename,errno);
        return;
    }


   OSI_PRINTFI("[%s %d] audio write image in sdcard", __FUNCTION__, __LINE__);
   ssize_t size = vfs_write(fd, in_buff, len);
   if (size != len)
   {
       OSI_PRINTFI("[%s %d] audio write size error %d", __FUNCTION__, __LINE__, size);
   }
   vfs_close(fd);
   OSI_PRINTFI("[%s %d] audio write data size %d", __FUNCTION__, __LINE__, size);
}
...
static void prvThreadEntry(void *param)
{
        ...
        while (1)
        {
            if(Rp_pos != d->Wp)
            {
                ...
                if (Rp_pos < d->Wp)
                {
                    ...
                    vfs_write(d->fd, &(d->audio_buff[Rp_pos]), (d->Wp - Rp_pos));
                    Rp_pos = d->Wp;
                }
                else
                {
	...
	fd_save = vfs_open(Filename, O_RDONLY);
	read_buff = (uint8_t *)malloc(bytes);
	
	vfs_read(fd_save, read_buff, bytes);
	savetoSD("/sdcard0/openrec.amr", read_buff, bytes);
    fibo_thread_delete();
}
