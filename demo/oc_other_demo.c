...
int32_t dumpCallBack_ins(void *arg)
{
    OSI_PRINTFI("cb infos: %s", (char *)arg);
    return 0;
}
...
{	
    ...
    do{
        CUSTOM_CALLBACK_TYPE cb_s = {0};
        cb_s.idleCallBack = NULL;
        cb_s.dumpCallBack = dumpCallBack_ins;
        OSI_PRINTFI("fibo_oc_dump_cb_init");
        fibo_oc_dump_cb_init((void*)(&cb_s));
    }while(0);
