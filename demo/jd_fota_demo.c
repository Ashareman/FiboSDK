...
{
	int ret;
	char *url = "http://xatest.fibocom.com:8000/httpfota/test4/500K.txt";

	ret = fibo_jd_firmware_dl(url);
	if(ret < 0){
	...
	/*ret = fibo_jd_firmware_update();
	if(ret < 0){
		OSI_PRINTFE("fibo_firmware_update fail.");
		OSI_PRINTFE("error code is");
		goto fexit;
	}*/
...
{
	int ret;
	char *url = "http://47.110.234.36:8000/httpfota/wlr/fw_04_03.bin";

	ret = fibo_jd_firmware_ota(url);
	if(ret < 0){
...
{
    char *fw_version = fibo_get_sw_verno();
    if (0 == memcmp(fw_version, "16000.1051.00.06.02.02", strlen(fw_version)))
    {
        OSI_PRINTFE("[FOTA] 01->02 upgrade successful!");
    }
    else
    {
        //OSI_PRINTFE("[FOTA] 01->02 upgrade failed!");
        if(TEST_TYPE == 1)
