...
static int ssl_recv_unblock(INT32 sock, void *buf, INT32 size, INT32 timeout)
{
    ...
    if (ret > 0)
    {
        OSI_PRINTFI("recv data size:%d", ret);
        return ret;
    }
    else if (ret < 0)
    {
    ...
    if (ret < 0)
    {
        OSI_PRINTFI("select failed:%s", strerror(errno));
        return -1;
    }
    else if (ret == 0)
    {
        OSI_PRINTFI("select timeout");
        return -1;
    }
    else
    {
        OSI_PRINTFI("data coming");
...
///非阻塞模式
static void prvThreadEntry_SSL_UNBLOCK(void *param)
{
    OSI_LOGI(0, "application thread enter, param 0x%x", param);
    //srand(100);
    ...
    ///设置sock fd为非阻塞
    ret = fibo_sock_lwip_fcntl(fd, F_SETFL, fibo_sock_lwip_fcntl(fd, F_GETFL, 0) | O_NONBLOCK);
    ...
    while (total < sizeof(p))
    {
        ...
        total += ssl_recv_unblock(sock, buf, 3, 1000);
        OSI_LOGI(0, "fibossl sys_sock_recv %s", (char *)buf);
    }
...
void *appimg_enter(void *param)
{
    ...
    fibo_thread_create(prvThreadEntry_SSL_UNBLOCK, "sslthread", 1024 * 4, NULL, OSI_PRIORITY_NORMAL);
    return 0;
}
