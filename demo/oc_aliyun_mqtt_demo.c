...
static void prvThreadEntry(void *param)
{
	...
	memcpy(host, "iot-as-mqtt.cn-shanghai.aliyuncs.com", strlen("iot-as-mqtt.cn-shanghai.aliyuncs.com"));
	...
	ret = fibo_aliyunMQTT_Tls_enable(1); //support TLS or not, 1: support; 0: not support. default:0
	if (ret == false)
	{
		OSI_PRINTFI("aliyunmqtt [%s-%d], enable tls falied!", __FUNCTION__, __LINE__);	
		fibo_thread_delete();
		return;
	}
