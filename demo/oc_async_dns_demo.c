...
{
    {
	   ...
	   // for(int n=0;n<10;n++)
	   {
		   ret = fibo_async_getHostByName("www.baidu.com",&addr,1,0);
		   fibo_taskSleep(1000);
		   OSI_PRINTFI("[%s-%d] ret = %d , addr.u_addr.ip4.addr = %d", __FUNCTION__, __LINE__,ret,addr.u_addr.ip4.addr);
		   ret = fibo_async_getHostByName("www.qq.com",&addr,1,0);
		   fibo_taskSleep(1000);
		   OSI_PRINTFI("[%s-%d] ret = %d , addr.u_addr.ip4.addr = %ld", __FUNCTION__, __LINE__,ret,addr.u_addr.ip4.addr);
		   ret = fibo_async_getHostByName("www.sina.com.cn",&addr,1,0);
		   fibo_taskSleep(1000);
		   OSI_PRINTFI("[%s-%d] ret = %d , addr.u_addr.ip4.addr = %d", __FUNCTION__, __LINE__,ret,addr.u_addr.ip4.addr);
           ret = fibo_async_getHostByName("http://www.sina.com.cn",&addr,1,0);
		   fibo_taskSleep(1000);
		   OSI_PRINTFI("[%s-%d] ret = %d , addr.u_addr.ip4.addr = %d", __FUNCTION__, __LINE__,ret,addr.u_addr.ip4.addr);
	   }
...
static void sig_res_callback(GAPP_SIGNAL_ID_T sig, va_list arg)
{
	...
	{   
		...
		{
			OSI_PRINTFI("sig_res_callback, ipv4_addr is NULL");
		}
		...
		{
			OSI_PRINTFI("sig_res_callback, ipv6_addr is NULL");
		}
